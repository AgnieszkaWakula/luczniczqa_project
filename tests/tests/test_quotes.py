...
class TestQuotePositiveFlows:
# POSITIVE and POSITIVE EXTENDED
    # GET positive - 200
    def test_01_read_quotes_list_response_200(self, create_and_delete_quote):
        ...
        with soft_assertions():
            ...
            assert_that(response.json()["data"][0][0]).contains_key("id")
    ...
    with soft_assertions():
        ...
        assert_that(delete_quote.json()["message"]).is_equal_to("Quote deleted successfully")
        # assert get non-existing quote
        assert_that(get_non_existing_quote_with_id.json()["detail"]).is_equal_to(f"Quote with {quote_id} doesn't exist.")

    # POST, GET and verify is id exists

# @pytest.mark.quotes
# @pytest.mark.usefixtures("create_and_delete_quote")
# class TestQuoteNegativeFlows:
